...
import { z } from "zod";
import { authConfig } from "./config"; // Yuqoridagi config import qilinadi
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...authConfig, // Config qoidalarini bu yerga yoyib yuboramiz
  callbacks: {
      ...
      if (user) {
        const dbUser = await prisma.user.findUnique({
          ...
          include: { 
            roles: true, 
            xpLedger: true, 
            enrollments: { where: { status: "active" }, take: 1, select: { id: true } } 
          },
        });
        if (dbUser) {
